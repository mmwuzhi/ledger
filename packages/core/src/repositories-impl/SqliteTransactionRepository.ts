...
export class SqliteTransactionRepository implements ITransactionRepository {
  ...
  async create(input: CreateTransactionInput): Promise<Transaction> {
    ...
    await this.db.runAsync(
      `INSERT INTO transactions (id, type, amount, category_id, note, date, receipt_id, recurring_id, book_id, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, input.type, input.amount, input.categoryId, input.note ?? '', input.date, input.receiptId ?? null, input.recurringId ?? null, input.bookId ?? 'default', timestamp, timestamp]
    );
  ...
  async update(id: string, input: UpdateTransactionInput): Promise<Transaction> {
    ...
    if (input.receiptId !== undefined) { fields.push('receipt_id = ?'); values.push(input.receiptId); }
